...
def validar_producto(producto):
    ...
    if not nombre or not cantidad or not valor_unitario:
        return False
...
def validar_campos():
    if not empresa_var.get() or not fecha_var.get() or not proveedor_var.get() or \
       not direccion_proveedor_var.get() or not telefono_proveedor_var.get() or \
       not cedula_proveedor_var.get() or not cliente_var.get() or \
       not direccion_cliente_var.get() or not telefono_cliente_var.get() or \
       not cedula_cliente_var.get() or not forma_pago_var.get():
    ...
    for producto in productos:
        if not validar_producto(producto):
            messagebox.showwarning("Formato de Productos", "Cada producto debe seguir el formato: Producto, Cantidad, Valor Unitario.")
            return False
